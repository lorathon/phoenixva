...
class Schedule_Pending extends Admin_Controller
{
    ...
    {        
        ...
        // Unset fields
        $crud->unset_add_fields('created', 'modified');
        ...
        $crud->add_action('Activate Schedule', base_url('images/icons/success_16x16.png'), 'admin/schedule_pending/move_pending', '');
        ...
        $this->data['subview'] = 'admin/schedule/pending';        
        $this->load->view('admin/_layout_main', $this->data);
    }
    
    public function move_pending($id)
    {
        $_id = $this->uri->segment(4);
        
        if($this->schedule_m->move_pending($_id))
        {
            $this->session->set_flashdata('alert_message', 'Schedule Activated!');
            $this->session->set_flashdata('alert_type', 'success');
        }
        else
        {
            $this->session->set_flashdata('alert_message', 'Schedule was NOT Activated!');
            $this->session->set_flashdata('alert_type', 'error');
        }  
        redirect('admin/schedule_pending');
    }
}